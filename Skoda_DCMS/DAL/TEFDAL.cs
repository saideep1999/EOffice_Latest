...
namespace Skoda_DCMS.DAL
{
    public class TEFDAL : CommonDAL
    {
        ...
        public async Task<ResponseModel<object>> SaveTEF(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();
            var listName = GlobalClass.ListNames.ContainsKey("TEF") ? GlobalClass.ListNames["TEF"] : "";
            string formShortName = "TEF";
            if (listName == "")
            {
                result.Status = 500;
            ...
            int RowId;
            string formName = "Training Evaluation Form";
            try
            {
                ...
                var response = GetApprovalTEF(empNum, ccNum, empLocationId);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "TEF"));
                ...
                cmd = new SqlCommand("USP_UpdateTEForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId ", userDetailsResponse.RowId));
                cmd.Parameters.Add(new SqlParameter("@TEFEmployeeID", form["TEFEmployeeID"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@TrainingProgramTitle", form["TrainingProgramTitle"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@TrainingDate", form["TrainingDate"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@TrainerName", form["TrainerName"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@OverallRating", form["OverallRating"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@Likes", form["Likes"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@TrainerRating", form["TrainerRating"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@ContentRating", form["ContentRating"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@FacilitiesRating", form["FacilitiesRating"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@ImprovementAreas", form["ImprovementAreas"] ?? (object)DBNull.Value));
        ...
        public ResponseModel<List<ApprovalMatrix>> GetApprovalTEF(long EmpNum, long CCNum, int LocationId)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_TEFApproval", con);
...
>>>>>>> latestcode25102024
        ...
        public async Task<dynamic> ViewTEFormData(int rowId, int formId)
        {
            dynamic TEFDataList = new ExpandoObject();
            try
            {
                ...
                List<TEFModel> item = new List<TEFModel>();
                ...
                TEFModel model = new TEFModel();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewTEFormDetails", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.TEFEmployeeID = Convert.ToString(dt.Rows[0]["TEFEmployeeID"]);
                        model.TrainingProgramTitle = Convert.ToString(dt.Rows[0]["TrainingProgramTitle"]);
                        model.TrainingDate = Convert.ToDateTime(dt.Rows[0]["TrainingDate"]);
                        model.TrainerName = Convert.ToString(dt.Rows[0]["TrainerName"]);
                        model.OverallRating = Convert.ToInt32(dt.Rows[0]["OverallRating"]);
                        model.Likes = Convert.ToString(dt.Rows[0]["Likes"]);
                        model.TrainerRating = Convert.ToInt32(dt.Rows[0]["TrainerRating"]);
                        model.ContentRating = Convert.ToInt32(dt.Rows[0]["ContentRating"]);
                        model.FacilitiesRating = Convert.ToInt32(dt.Rows[0]["FacilitiesRating"]);
                        model.ImprovementAreas = Convert.ToString(dt.Rows[0]["ImprovementAreas"]);
                        item.Add(model);
                    }
                }
                TEFDataList.one = item;
                ...
                if (r1.Status == 500)
                    ...
                    return r2;
                TEFDataList.two = r1.Model;
                TEFDataList.three = r2.Model;
