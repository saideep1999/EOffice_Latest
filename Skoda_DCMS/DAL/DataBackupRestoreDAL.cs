...
{
    public class DataBackupRestoreDAL
    {
        ...
        /// </summary>
        /// <returns></returns>
        // public async Task<dynamic> CreateDataBackupRestoreRequest(System.Web.Mvc.FormCollection form, UserData user)
        public async Task<ResponseModel<object>> CreateDataBackupRestoreRequest(System.Web.Mvc.FormCollection form, UserData user)
        {
            ...
            try
            {
                ...
                var response = await GetApprovalForDataBackupRestore(empNum, ccNum, loc);
                ...
                if (FormId == 0)
                {
                    ...
                    item["FormName"] = "Data Backup Restore Form";
                    item["UniqueFormName"] = "DBRF";
                    item["FormParentId"] = 13;
                    item["ListName"] = listName;
                    item["SubmitterUserName"] = user.UserName;
                    item["Status"] = "Submitted";
                    item["TimeStamp"] = DateTime.Now.ToString("yyyy-MM-ddThh:mm:ss");
                    item["Department"] = user.Department;
                    item["ControllerName"] = "DataBackupRestore";
                ...
                newRow["RequirementFor"] = form["chkRequirementFor"] ?? "";
                newRow["RequestFor"] = form["chkRequestFor"] ?? "";
                newRow["FolderPath"] = form["txtFolderPath"] ?? "";
                newRow["FolderSize"] = form["txtFolderSize"] ?? "";
                newRow["RetentionPeriod"] = form["txtRetentionPeriod"] == "" ? null : form["txtRetentionPeriod"];
                newRow["BackupServerName"] = form["txtBackupServerName"];
                newRow["BackupIpAddress"] = form["txtBackupIpAddress"];
                newRow["BackupType"] = form["chkBackupType"] ?? "";
                newRow["RestoreAt"] = form["chkRestoreAt"] ?? "";
                newRow["RestoreServerName"] = form["txtRestoreServerName"] ?? "";
                newRow["RestoreIpAddress"] = form["txtRestoreIpAddress"] ?? "";
                newRow["RestoreDate"] = form["txtRestoreDate"] == "" ? null : form["txtRestoreDate"];
                newRow["AlternateFolderPath"] = form["txtAlternateFolderPath"] ?? "";
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetApprovalForDataBackupRestore(long empNum, long ccNum, string loc)
        {
            try
            {
                ...
                cmd = new SqlCommand("sp_DataBackupRestoreApproval", con);
        ...
        /// <summary>
        /// DataBackuPRestore-It is used for viewing software requisition form.
        /// </summary>
        /// <returns></returns>
        public async Task<dynamic> GetDataBackuPRestoreDetails(int rowId, int formId)
        {
            dynamic dataBackupRestore = new ExpandoObject();
            try
            {
                ...
                var response = await client.GetAsync("_api/web/lists/GetByTitle('DataBackupRestore')/items?$select=ID,EmployeeType,EmployeeCode," +
                    ...
                    "RestoreDate,AlternateFolderPath,EmployeeLocation,ExternalOrganizationName,ExternalOtherOrganizationName,RequestSubmissionFor,FormID/ID,FormID/Created&$filter=(ID eq '" + rowId + "')&$expand=FormID");
                ...
                {
                    var result = JsonConvert.DeserializeObject<DataBackupRestoreModel>(responseText, settings);
                    dataBackupRestore.one = result.List.DataBackupRestoreList;
                }
                ...
                var response2 = await client2.GetAsync("_api/web/lists/GetByTitle('ApprovalMaster')/items?$select=ApproverId,ApproverStatus,Modified,IsActive,Comment,NextApproverId,Level,ApproverName,ApproverUserName,Logic,TimeStamp,Designation,"
                ...
                if (modelData.Node.Data.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(responseText2))
                    {
                        ...
                        dataBackupRestore.two = data2.d.results;
                        dataBackupRestore.three = items;
                    }

                }
                else
                {
                    dataBackupRestore.two = new List<string>();
                    dataBackupRestore.three = new List<string>();
                }
