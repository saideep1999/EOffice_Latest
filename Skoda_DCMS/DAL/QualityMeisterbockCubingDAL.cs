...
using Attachment = Microsoft.SharePoint.Client.Attachment;

namespace Skoda_DCMS.DAL
{
    public class QualityMeisterbockCubingDAL : CommonDAL
    {
        ...
        public async Task<ResponseModel<object>> SaveQMCRForm(QualityMeisterbockCubingData model, UserData user, HttpPostedFileBase file, HttpPostedFileBase file1)
        {
            ...
            var response = await GetApprovalQMRCForm(empNum, ccNum, empDes, model);
            ...
            try
            {
                ...
                if (formId == 0)
                {
                    ...
                    item["ControllerName"] = "QualityMeisterbockCubingform";
                ...
                newRow["FormType"] = model.FormType;
                newRow["ModelQCM"] = model.ModelQCM;
                newRow["Series"] = model.Series;
                newRow["PartQuantity"] = model.PartQuantity;
                newRow["ProblemSheet"] = model.ProblemSheet;
                newRow["TrialReported"] = model.TrialReported;
                newRow["BusinessNeed"] = model.BusinessNeed;
                newRow["Details"] = model.Details;
                newRow["PartName"] = model.PartName;
                newRow["ProblemReported"] = model.ProblemReported;
                newRow["OtherDetails"] = model.OtherDetails;
                ...
                if (attachedfile != null && attachedfile != "")
                {
                    ...
                    {
                        var check = oFile.Name;
                        var resultFileName = check.Substring(0, 2);
                        if (resultFileName == "PS")
                        {
                ...
                if (file1 != null)
                {
                    ...
                    {
                        fileData = binaryReader.ReadBytes(file1.ContentLength);
                    }
                ...
                var attachedfile1 = model.attachedfile1;
                if (attachedfile1 != null && attachedfile1 != "")
                {
                    ...
                    var attachedfileName = model.attachedfileName1;
                    int attachmentID = RowId;
        ...
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetApprovalQMRCForm(long empNum, long ccNum, string empDes, QualityMeisterbockCubingData model)
        {
            try
            {
                ...
                cmd = new SqlCommand("sp_QMRCForm", con);
                ...
                cmd.Parameters.Add(new SqlParameter("@ActionType", model.FormType));
        ...
        public async Task<dynamic> ViewQualityMeisterbockCubingFormData(int rowId, int formId)
        {
            ...
            try
            {
                ...
                var response1 = await client1.GetAsync("_api/web/lists/GetByTitle('QualityMeisterbockCubingList')/items?$select=*"
  + "&$filter=(ID eq '" + rowId + "')&$expand=AttachmentFiles");
                ...
                {
                    var SUCFUserResult = JsonConvert.DeserializeObject<QualityMeisterbockCubingModel>(responseText1, settings);
                    URCFData.one = SUCFUserResult.QualityMeisterbockCubingResults.QualityMeisterbockCubingData;
                }
        ...
        public async Task<int> UpdateData(QualityMeisterbockCubingData model, UserData user)
        {
            ...
            try
            {
                ...
                newItem["StartTime"] = model.StartTime.Value.ToString("yyyy-MM-ddThh:mm:ss");
                newItem["EndTime"] = model.EndTime.Value.ToString("yyyy-MM-ddThh:mm:ss");
                newItem.Update();
