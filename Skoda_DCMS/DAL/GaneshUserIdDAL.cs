...
namespace Skoda_DCMS.DAL
{
    public class GaneshUserIdDAL
    {
        ...
        /// <summary>
        /// Ganesh User Id Creation-It is used to save data in sharepoint list.
        /// </summary>
        /// <returns></returns>
        //public async Task<dynamic> CreateGaneshUserIdCreationRequest(System.Web.Mvc.FormCollection form, UserData user)
        public async Task<ResponseModel<object>> CreateGaneshUserIdCreationRequest(System.Web.Mvc.FormCollection form, UserData user)
        {
            ...
            try
            {
                ...
                var response = await GetApprovalForGetGaneshUserIdCreation(empNum, ccNum);
                ...
                if (FormId == 0)
                {
                    ...
                    item["FormName"] = "Ganesh User Id Creation Form";
                    item["UniqueFormName"] = "GUICF";
                    item["FormParentId"] = 11;
                    item["ListName"] = listName;
                    item["SubmitterUserName"] = user.UserName;
                    item["Status"] = "Submitted";
                    item["TimeStamp"] = DateTime.Now.ToString("yyyy-MM-ddThh:mm:ss");
                    item["Department"] = user.Department;
                    item["ControllerName"] = "GaneshUserId";
                ...
                newRow["IsCreationRequest"] = form["chkCreateUserId"] ?? "";
                newRow["IsRoleAuthRequest"] = form["chkRoleAuthorization"] ?? "";
                newRow["SystemType"] = form["chkSystemType"];
                ...
                int CreationId = newRow.Id;

                List GaneshModuleList = web.Lists.GetByTitle("GaneshModuleDetails");

                string pattern = ",";

                var ModuleName = form["txtModulePost"];
                var Role = form["txtRolePost"];
                var Reason = form["txtReasonPost"];

                var moduleNames = ModuleName.Split(new string[] { pattern }, StringSplitOptions.None).ToList();
                moduleNames = moduleNames.Where(s => !string.IsNullOrEmpty(s)).ToList();
                var roles = Role.Split(new string[] { pattern }, StringSplitOptions.None).ToList();
                roles = roles.Where(s => !string.IsNullOrEmpty(s)).ToList();
                var reasons = Reason.Split(new string[] { pattern }, StringSplitOptions.None).ToList();
                reasons = reasons.Where(s => !string.IsNullOrEmpty(s)).ToList();

                for (int i = 0; i < moduleNames.Count; i++)
                {
                    ListItemCreationInformation itemCreate = new ListItemCreationInformation();
                    ListItem newFormItem = GaneshModuleList.AddItem(itemCreate);
                    newFormItem["GaneshUserReqID"] = CreationId;
                    newFormItem["ModuleName"] = moduleNames[i].Replace("|!", ",");
                    newFormItem["Role"] = roles[i].Replace("|!", ",");
                    newFormItem["Reason"] = reasons[i].Replace("|!", ",");
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<dynamic> GetGaneshUserIdCreationDetails(int rowId, int formId)
        {
            ...
            try
            {
                ...
                var response = await client.GetAsync("_api/web/lists/GetByTitle('GaneshUserIdCreation')/items?$select=ID,EmployeeType,EmployeeCode," +
                    ...
                    "IsRoleAuthRequest,EmployeeRequestType,TempFrom,TempTo,EmployeeLocation,RequestSubmissionFor,FormID/ID,FormID/Created&$filter=(ID eq '" + rowId + "')&$expand=FormID");
                ...
                {
                    var result = JsonConvert.DeserializeObject<GaneshUserIdCreationModel>(responseText, settings);
                    userIdCreation.one = result.List.GaneshUserIdList;
                }
                ...
                var response2 = await client2.GetAsync("_api/web/lists/GetByTitle('GaneshModuleDetails')/items?$select=ModuleName,Role,Reason&$filter=(GaneshUserReqID eq '" + rowId + "')");
                var responseText2 = await response2.Content.ReadAsStringAsync();

                var result1 = JsonConvert.DeserializeObject<GaneshModuleAccessModel>(responseText2);
                userIdCreation.two = result1.List.GaneshModuleList;
                userIdCreation.Count = userIdCreation.two.Count;
                ...
                if (modelData.Node.Data.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(responseText3))
                    {
                        ...
                        userIdCreation.three = data3.d.results;
                        userIdCreation.four = items;
                    }
                }
                else
                {
                    ...
                    userIdCreation.four = new List<string>();
                }
        ...
        /// <summary>
        /// GetGaneshUserIdCreation-It is used for getting approvers from sql db.
        /// </summary>
        /// <returns></returns>
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetApprovalForGetGaneshUserIdCreation(long empNum, long ccNum)
        {
            try
            {
                ...
                cmd = new SqlCommand("sp_GaneshUserIdCreationApproval", con);
