...
namespace Skoda_DCMS.DAL
{
    public class ERFDAL : CommonDAL
    {
        ...
        public async Task<ResponseModel<object>> SaveERF(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();
            var listName = GlobalClass.ListNames.ContainsKey("ERF") ? GlobalClass.ListNames["ERF"] : "";
            string formShortName = "ERF";
            if (listName == "")
            {
                result.Status = 500;
            ...
            int RowId;
            string formName = "Expense Reimbursement Form";
            try
            {
                ...
                var response = GetApprovalERF(empNum, ccNum, empLocationId);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "ERF"));
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@FormParentId", 59));
                ...
                cmd = new SqlCommand("USP_UpdateERForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId ", userDetailsResponse.RowId));
                cmd.Parameters.Add(new SqlParameter("@ERFEmployeeID", form["ERFEmployeeID"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@ExpenseType", form["ExpenseType"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@ExpenseDate", form["ExpenseDate"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@ExpenseAmount", form["ExpenseAmount"] ?? (object)DBNull.Value));
                if (form["ReceiptAttached"] == "on")
                    cmd.Parameters.Add(new SqlParameter("@ReceiptAttached",  true ));
                else
                    cmd.Parameters.Add(new SqlParameter("@ReceiptAttached", false));
                cmd.Parameters.Add(new SqlParameter("@CostCode", form["CostCode"] ?? (object)DBNull.Value));
        ...
        public ResponseModel<List<ApprovalMatrix>> GetApprovalERF(long EmpNum, long CCNum, int LocationId)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_ERFApproval", con);
        ...
        public async Task<dynamic> ViewERFormData(int rowId, int formId)
        {
            dynamic ERFDataList = new ExpandoObject();
            try
            {
                ...
                cmd = new SqlCommand("USP_ViewERFormDetails", conn);
                ...
                if (r1.Status == 500)
                    ...
                    return r2;
                ERFDataList.two = r1.Model;
                ERFDataList.three = r2.Model;
