...
namespace Skoda_DCMS.DAL
{
    public class LAFDAL : CommonDAL
    {
        ...
        public async Task<ResponseModel<object>> SaveLAF(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();
            var listName = GlobalClass.ListNames.ContainsKey("LAF") ? GlobalClass.ListNames["LAF"] : "";
            string formShortName = "LAF";
            if (listName == "")
            {
                result.Status = 500;
            ...
            int RowId;
            string formName = "Leave Application Form";
            try
            {
                ...
                var response = GetApprovalLAF(empNum, ccNum, empLocationId);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "LAF"));
                ...
                cmd = new SqlCommand("USP_UpdateLAForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId ", userDetailsResponse.RowId));
                cmd.Parameters.Add(new SqlParameter("@LAFEmployeeID", form["LAFEmployeeID"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@LeaveType", form["LeaveType"] ?? (object)DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@LeaveStartDate", !string.IsNullOrEmpty(form["LeaveStartDate"]) ? (object)Convert.ToDateTime(form["LeaveStartDate"]) : DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@LeaveEndDate", !string.IsNullOrEmpty(form["LeaveEndDate"]) ? (object)Convert.ToDateTime(form["LeaveEndDate"]) : DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@TotalLeaveDays", !string.IsNullOrEmpty(form["TotalLeaveDays"]) ? (object)Convert.ToInt32(form["TotalLeaveDays"]) : DBNull.Value));
                cmd.Parameters.Add(new SqlParameter("@LeaveReason", form["LeaveReason"] ?? (object)DBNull.Value));
        ...
        public ResponseModel<List<ApprovalMatrix>> GetApprovalLAF(long EmpNum, long CCNum, int LocationId)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_LAFApproval", con);
        ...
        public async Task<dynamic> ViewLAFormData(int rowId, int formId)
        {
            dynamic LAFDataList = new ExpandoObject();
            try
            {
                ...
                List<LAFModel> item = new List<LAFModel>();
                ...
                LAFModel model = new LAFModel();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewLAFormDetails", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.LAFEmployeeID = Convert.ToString(dt.Rows[0]["LAFEmployeeID"]);
                        model.LeaveType = Convert.ToString(dt.Rows[0]["LeaveType"]);
                        model.LeaveStartDate = dt.Rows[0]["LeaveStartDate"] != DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["LeaveStartDate"]) : (DateTime?)null;
                        model.LeaveEndDate = dt.Rows[0]["LeaveEndDate"] != DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["LeaveEndDate"]) : (DateTime?)null;
                        model.TotalLeaveDays = dt.Rows[0]["TotalLeaveDays"] != DBNull.Value ? Convert.ToInt32(dt.Rows[0]["TotalLeaveDays"]) : 0;
                        model.LeaveReason = Convert.ToString(dt.Rows[0]["LeaveReason"]);
                        item.Add(model);
                    }
                }
                LAFDataList.one = item;
                ...
                if (r1.Status == 500)
                    ...
                    return r2;
                LAFDataList.two = r1.Model;
                LAFDataList.three = r2.Model;
