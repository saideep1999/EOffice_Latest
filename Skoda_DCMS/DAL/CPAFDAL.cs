...
{
    public class CPAFDAL : CommonDAL
    {
        ...
        public async Task<ResponseModel<object>> SaveCPAF(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();
            var listName = GlobalClass.ListNames.ContainsKey("CPAF") ? GlobalClass.ListNames["CPAF"] : "";
            string formShortName = "CPAF";
            if (listName == "")
            {
                result.Status = 500;
            ...
            int RowId;
            string formName = "Company Policy Acknowledgment Form";
            try
            {
                ...
                var response = GetApprovalCPAF(empNum, ccNum, empLocationId);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "CPAF"));
                ...
                cmd = new SqlCommand("USP_UpdateCPAForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId ", userDetailsResponse.RowId));
                cmd.Parameters.Add(new SqlParameter("@CPAFEmployeeID", form["CPAFEmployeeID"] ?? (object)DBNull.Value)); 
                cmd.Parameters.Add(new SqlParameter("@FullName", form["FullName"] ?? (object)DBNull.Value)); // Full Name
                cmd.Parameters.Add(new SqlParameter("@Department", form["Department"] ?? (object)DBNull.Value)); // Department
                cmd.Parameters.Add(new SqlParameter("@JobTitle", form["JobTitle"] ?? (object)DBNull.Value)); // Job Title
                cmd.Parameters.Add(new SqlParameter("@AcknowledgementDate",
                    !string.IsNullOrEmpty(form["AcknowledgementDate"])
                    ? (object)DateTime.Parse(form["AcknowledgementDate"])
                    : DBNull.Value)); // Acknowledgement Date
                cmd.Parameters.Add(new SqlParameter("@AcknowledgementStatement", form["AcknowledgementStatement"] ?? (object)DBNull.Value)); // Acknowledgement Statement
        ...
        public ResponseModel<List<ApprovalMatrix>> GetApprovalCPAF(long EmpNum, long CCNum, int LocationId)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_CPAFApproval", con);
        ...
        public async Task<dynamic> ViewCPAFormData(int rowId, int formId)
        {
            dynamic CPAFDataList = new ExpandoObject();
            try
            {
                ...
                List<CPAFModel> item = new List<CPAFModel>();
                ...
                CPAFModel model = new CPAFModel();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewCPAFormDetails", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.CPAFEmployeeID = Convert.ToString(dt.Rows[0]["CPAFEmployeeID"]); // Employee ID
                        model.FullName = Convert.ToString(dt.Rows[0]["FullName"]); // Full Name
                        model.Department = Convert.ToString(dt.Rows[0]["Department"]); // Department
                        model.JobTitle = Convert.ToString(dt.Rows[0]["JobTitle"]); // Job Title
                        model.AcknowledgementDate = dt.Rows[0]["AcknowledgementDate"] == DBNull.Value
                                                    ? (DateTime?)null
                                                    : Convert.ToDateTime(dt.Rows[0]["AcknowledgementDate"]); // Acknowledgement Date
                        model.AcknowledgementStatement = Convert.ToString(dt.Rows[0]["AcknowledgementStatement"]); // Acknowledgement Statement
                        item.Add(model);
                    }
                }
                CPAFDataList.one = item;
                ...
                if (r1.Status == 500)
                    ...
                    return r2;
                CPAFDataList.two = r1.Model;
                CPAFDataList.three = r2.Model;
