...
{
    public class ReissueIDCardDAL : CommonDAL
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ResponseModel<object>> CreateReissueIDCardRequest(System.Web.Mvc.FormCollection form, UserData user, HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                var response = await GetApprovalForReissueIDCard(empNum, ccNum, loc, isPKI, isInternal, externlOrgName);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "ReissueIDCard"));
                ...
                if (file != null)
                {
                    if (file.ContentLength > 0)
                    {
                        ...
                        _path = Path.Combine(System.Web.HttpContext.Current.Server.MapPath("~/UploadPhotoReissueID"), _FileName);
                        _pathfORSaving = "/UploadPhotoReissueID/" + _FileName;
                        file.SaveAs(_path);
                ...
                cmd = new SqlCommand("USP_UpdateReissueIDCardForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId ", userDetailsResponse.RowId));
                cmd.Parameters.Add(new SqlParameter("@DateofJoining", form["txtDateofJoining"]));
                cmd.Parameters.Add(new SqlParameter("@ReasonforReissue", form["drpReasonforReissue"]));
                cmd.Parameters.Add(new SqlParameter("@TypeOfCard", form["chkTypeOfCard"]));
                cmd.Parameters.Add(new SqlParameter("@BusinessNeed", form["txtBusinessNeed"]));
                cmd.Parameters.Add(new SqlParameter("@OtherReason", form["drpReasonforReissue"] == "Others" ? form["txtReasonforReissueOther"] : ""));
                cmd.Parameters.Add(new SqlParameter("@UploadPhoto", _pathfORSaving));
        ...
        /// <summary>
        /// ReissueIDCard-It is used for viewing Reissue ID Card form.
        /// </summary>
        /// <returns></returns>
        public async Task<dynamic> GetReissueIDCardDetails(int rowId, int formId)
        {
            ...
            try
            {
                ...
                List<ReissueIDCardData> item = new List<ReissueIDCardData>();
                AttachmentFilesResults attachmentFilesResults = new AttachmentFilesResults();
                ReissueIDCardData model = new ReissueIDCardData();
                ReissueIDCardResults results = new ReissueIDCardResults();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewReissueIDCardForm", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.ReasonforReissue = Convert.ToString(dt.Rows[0]["ReasonforReissue"]);
                        model.BusinessNeed = Convert.ToString(dt.Rows[0]["BusinessNeed"]);
                        model.OtherReason = Convert.ToString(dt.Rows[0]["OtherReason"]);
                        model.UploadPhoto = Convert.ToString(dt.Rows[0]["UploadPhoto"]);
                        model.TypeOfCard = Convert.ToString(dt.Rows[0]["TypeOfCard"]);
                        item.Add(model);
                    }
                }
        ...
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetApprovalForReissueIDCard(long empNum, long ccNum, string loc, int isPKI, int isInternal, string externlOrgName)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_ReissueIDCardApproval", con);
        ...
        public int RIDCFValidityUpdate(System.Web.Mvc.FormCollection form, UserData user)
        {
            ...
            try
            {
                ...
                cmd1 = new SqlCommand("UpdateReissueIDCardForm", con);
                cmd1.Parameters.Add(new SqlParameter("@FormSrId", rowId));
                cmd1.Parameters.Add(new SqlParameter("@DateofIssue", Convert.ToDateTime(form["txtDateOfIssue"])));
                cmd1.Parameters.Add(new SqlParameter("@Chargeable", Convert.ToInt32(form["drpChargeable"].ToLower() == "yes" ? 1 : 0)));
                cmd1.Parameters.Add(new SqlParameter("@IDCardNumber", form["txtIDCardNumber"]));
                // cmd.Parameters.Add(new SqlParameter("@FutureOwnerEmail", model.FutureOwnerEmail));
                cmd1.CommandType = CommandType.StoredProcedure;
                adapter1.SelectCommand = cmd1;
                con.Open();
        ...
        public async Task<dynamic> ReasonforReissue()
        {
            List<ReissueIDCardData> list = new List<ReissueIDCardData>();
            try
            {
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("sp_GetReasonforReissue", con);
                cmd.CommandType = CommandType.StoredProcedure;
                adapter.SelectCommand = cmd;
                con.Open();
                adapter.Fill(ds);
                ...
                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0] != null)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ReissueIDCardData reissueIDCard = new ReissueIDCardData();
                        reissueIDCard.ReasonforReissue = Convert.ToString(ds.Tables[0].Rows[i]["ReasonforReissue"]);
                        list.Add(reissueIDCard);
                    }
                }
            }
            catch (Exception ex)
