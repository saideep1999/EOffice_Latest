...
{
    public class SalaryReviewAdjustmentDAL : CommonDAL
    {
        ...
        public async Task<dynamic> GetSRAFDetails(int rowId, int formId)
        {
            ...
            try
            {
                ...
                List<SalaryReviewAdjustmentData> item = new List<SalaryReviewAdjustmentData>();
                AttachmentFilesResults attachmentFilesResults = new AttachmentFilesResults();
                SalaryReviewAdjustmentData model = new SalaryReviewAdjustmentData();
                TrainingRequestResults results = new TrainingRequestResults();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewSalaryReviewAdjustmentForm", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.CurrentSalary = Convert.ToDecimal(dt.Rows[0]["CurrentSalary"]);
                        model.RequestedSalary = Convert.ToDecimal(dt.Rows[0]["RequestedSalary"]);
                        model.EffectiveDate = Convert.ToDateTime(dt.Rows[0]["EffectiveDate"]);
                        model.SalaryJustification = Convert.ToString(dt.Rows[0]["SalaryJustification"]);
                        item.Add(model);
                    }
                }
        ...
        public async Task<ResponseModel<object>> SaveSRAFDAL(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();

            int RowId = 0;
            string formShortName = "SRAF";
            string formName = "Salary Review Adjustment";
            ...
            try
            {
                ...
                var response = await GetSalaryReviewAdjustmentApprovers(empNum, ccNum, loc, isPKI, isInternal, externlOrgName);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "SalaryReviewAdjustment"));
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@FormParentId", 62));
                ...
                cmd = new SqlCommand("USP_UpdateSalaryReviewAdjustmentForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId", RowId));
                cmd.Parameters.Add(new SqlParameter("@EmployeeID", form["EmployeeID"])); // Employee ID field
                cmd.Parameters.Add(new SqlParameter("@Department", form["Department"])); // Department field
                cmd.Parameters.Add(new SqlParameter("@JobTitle", form["JobTitle"])); // Job Title field
                cmd.Parameters.Add(new SqlParameter("@CurrentSalary", decimal.Parse(form["CurrentSalary"]))); // Current Salary (decimal)
                cmd.Parameters.Add(new SqlParameter("@RequestedSalary", decimal.Parse(form["RequestedSalary"]))); // Requested Salary (decimal)
                cmd.Parameters.Add(new SqlParameter("@EffectiveDate", DateTime.Parse(form["EffectiveDate"]))); // Converting string to DateTime for Effective Date
                cmd.Parameters.Add(new SqlParameter("@SalaryJustification", form["SalaryJustification"])); // Justification for Salary Increase
        ...
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetSalaryReviewAdjustmentApprovers(long empNum, long ccNum, string loc, int isPKI, int isInternal, string externlOrgName)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_SARFApproval", con);
