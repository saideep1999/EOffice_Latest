...
namespace Skoda_DCMS.DAL
{
    public class TRFDAL : CommonDAL
    {
        ...
        public async Task<dynamic> GetTRFDetails(int rowId, int formId)
        {
            ...
            try
            {
                ...
                List<TrainingRequestData> item = new List<TrainingRequestData>();
                AttachmentFilesResults attachmentFilesResults = new AttachmentFilesResults();
                TrainingRequestData model = new TrainingRequestData();
                TrainingRequestResults results = new TrainingRequestResults();
                SqlCommand cmd = new SqlCommand();
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataTable dt = new DataTable();
                var conn = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_ViewTrainingRequestForm", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        model.ReasonForTraining = Convert.ToString(dt.Rows[0]["ReasonForTraining"]);
                        ...
                        model.TrainingProvider = Convert.ToString(dt.Rows[0]["TrainingProvider"]);
                        model.ExpectedOutcomes = Convert.ToString(dt.Rows[0]["ExpectedOutcomes"]);
                        model.CompletionStatus = Convert.ToString(dt.Rows[0]["CompletionStatus"]);
                        model.Assessment = Convert.ToString(dt.Rows[0]["Assessment"]);
        ...
        public async Task<ResponseModel<object>> SaveTRFDAL(System.Web.Mvc.FormCollection form, UserData user)
        {
            ResponseModel<object> result = new ResponseModel<object>();

            int RowId = 0;
            string formShortName = "TRF";
            string formName = "Training Request Form";
            ...
            try
            {
                ...
                var response = await GetTrainingRequestApprovers(empNum, ccNum, loc, isPKI, isInternal, externlOrgName);
                ...
                if (FormId == 0)
                {
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@ControllerName", "Training"));
                    ...
                    cmd_form.Parameters.Add(new SqlParameter("@FormParentId", 57));
                ...
                cmd = new SqlCommand("USP_UpdatTrainingRequestForm", con);
                cmd.Parameters.Add(new SqlParameter("@FormID", formId));
                cmd.Parameters.Add(new SqlParameter("@RowId", RowId));
                cmd.Parameters.Add(new SqlParameter("@EmployeeID", form["EmployeeID"])); // Assuming EmployeeID is the form field for Employee ID
                cmd.Parameters.Add(new SqlParameter("@TrainingProgramTitle", form["TrainingProgramTitle"])); // Training Program Title field
                cmd.Parameters.Add(new SqlParameter("@TrainingProvider", form["TrainingProvider"])); // Training Provider field
                ...
                cmd.Parameters.Add(new SqlParameter("@ReasonForTraining", form["ReasonForTraining"])); // Reason for Training field
                cmd.Parameters.Add(new SqlParameter("@ExpectedOutcomes", form["ExpectedOutcomes"])); // Expected Outcomes field
                cmd.Parameters.Add(new SqlParameter("@CompletionStatus", form["CompletionStatus"])); // Completion Status (auto-populated)
                cmd.Parameters.Add(new SqlParameter("@Assessment", form["Assessment"])); // Assessment (auto-populated)
        ...
        public async Task<ResponseModel<List<ApprovalMatrix>>> GetTrainingRequestApprovers(long empNum, long ccNum, string loc, int isPKI, int isInternal, string externlOrgName)
        {
            try
            {
                SqlCommand cmd = new SqlCommand();
                ...
                con = new SqlConnection(sqlConString);
                cmd = new SqlCommand("USP_TRFApproval", con);
